...
def read_process_gpu_pcts(
    ...
    if not probe_state.can_probe():
        return {}
    ok, _ = refresh_sudo_credentials(prompt=False)
    if not ok:
        return {}
    try:
        text_proc = subprocess.run(
            process_gpu_command(sample_ms, "text"),
            check=False,
            capture_output=True,
...
def read_processes(include_gpu: bool = False) -> list[ProcessInfo]:
    ...
    for command in commands:
        ...
        if processes:
            break
    if include_gpu and processes:
        processes = merge_process_gpu_pcts(processes, read_process_gpu_pcts())
    return processes
