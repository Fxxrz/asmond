...
def read_battery_items() -> dict[str, Any] | None:
    try:
        proc = subprocess.run(
            [IOREG_PATH, "-a", "-r", "-c", "AppleSmartBattery"],
            check=False,
            capture_output=True,
            timeout=1.0,
...
def read_hpm_port_items() -> list[dict[str, Any]]:
    try:
        proc = subprocess.run(
            [IOREG_PATH, "-a", "-r", "-c", "AppleHPMInterface"],
            check=False,
            capture_output=True,
            timeout=1.0,
            env=system_environment(),
...
def ensure_powermetrics_access(args: argparse.Namespace) -> None:
    if args.mock or os.name != "posix" or is_root_process():
        return
    print(f"{APP_NAME} keeps the UI unprivileged and asks sudo only for powermetrics.", file=sys.stderr)
...
def apply_system_cpu_usage(sample: MetricSample, cpu_usage_pct: float | None) -> bool:
    if cpu_usage_pct is None:
        return False
    if sample.telemetry_source == "intel":
...
def run_curses(stdscr: curses.window, args: argparse.Namespace) -> None:
    ...
    def io_poll_visible() -> bool:
        ...
        layout = dashboard_layout(max_y, max_x, args.layout, args.show_io, process_panel, args.custom_layout)
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    has_telemetry = next_sample.raw_keys > 0 or any_present(
                        next_sample.cpu_power_mw,
                        next_sample.gpu_power_mw,
                        next_sample.soc_power_mw,
                        next_sample.cpu_usage_pct,
                        next_sample.gpu_usage_pct,
                        next_sample.soc_temp_c,
                    )
                    if next_sample.warning and not has_telemetry:
                        status = f"sampler warning: {next_sample.warning.splitlines()[0]}"
                        continue
                    latest = next_sample
...
def run_probe(args: argparse.Namespace) -> int:
    ...
    if proc.returncode != 0:
        ...
        print(sanitize_terminal_text(detail), file=sys.stderr)
    ...
    if args.raw:
        ...
        for path, value in sorted(flatten(obj), key=lambda item: item[0].lower()):
            ...
            print(sanitize_terminal_text(f"{path}: {value}"))
    return 0
...
def print_sample(sample: MetricSample) -> None:
    ...
    print(sanitize_terminal_text(f"Thermal:      {sample.thermal_pressure or 'n/a'}"))
    print(f"Throttled:    {sample.throttled if sample.throttled is not None else 'unknown'}")
    if sample.throttle_reasons:
        print(sanitize_terminal_text(f"Limits:       {', '.join(sample.throttle_reasons)}"))
        ...
        if ok or is_root_process():
            ...
            try:
                ...
                if proc.returncode == 0 and raw:
                    try:
                        valid_plist = isinstance(plistlib.loads(raw), dict)
                    except Exception:
                        valid_plist = False
...
def run_doctor(args: argparse.Namespace) -> int:
    ...
    for name, status, note in rows:
        ...
        print(sanitize_terminal_text(f"{marker:<4} {name:<{width}}  {note}"))
    return diagnostics_exit_code(rows)
...
def print_markdown_report(data: dict[str, Any]) -> None:
    def safe_print(value: Any = "") -> None:
        print(sanitize_terminal_text(value))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if getattr(args, "settings_warning", ""):
        print(f"{APP_NAME}: {args.settings_warning}", file=sys.stderr)
