    ...
    if isinstance(value, str):
        ...
        if match:
            ...
            if math.isfinite(number):
                return number, (match.group(2) or "").lower() or None
    return None, None
...
def truthy_flag(value: Any) -> bool:
    ...
    if isinstance(value, str):
        return value.strip().lower() in {"1", "true", "yes", "active", "invalid"}
    return False


def without_invalid_blocks(obj: Any) -> tuple[Any | None, int]:
    """Remove plist mappings explicitly marked invalid by powermetrics."""
    if isinstance(obj, dict):
        if truthy_flag(obj.get("invalid")):
            return None, 1
        ...
        for key, value in obj.items():
            if key == "invalid":
                continue
            next_value, next_count = without_invalid_blocks(value)
            invalid_count += next_count
...
def as_mw(path: str, value: Any) -> float | None:
    ...
    raw_key = path.lower()
    if unit == "w" or " watts" in key or re.search(r"(?:^|[._\s-])w$", raw_key):
        return number * 1000.0
    if unit == "uw" or re.search(r"(?:^|[._\s-])uw$", raw_key):
        return number / 1000.0
    if unit == "mw" or " mw" in key or "milliwatt" in key or re.search(r"(?:^|[._\s-])mw$", raw_key):
        return number
...
def bandwidth_counters_from_plist(obj: dict[str, Any]) -> dict[str, float]:
    ...
    for item in counters:
        if not isinstance(item, dict):
            continue
        if truthy_flag(item.get("invalid")):
            continue
...
def temp_stats_from_flat(flat: list[tuple[str, Any]]) -> tuple[float | None, float | None]:
    ...
    for path, value in flat:
        ...
        if not tokens.intersection(include_words):
            continue
        if any(word in key for word in exclude_words):
            continue
        temp = as_temp_c(path, value)
        sensor_path = re.sub(r"\s+(temperature|current value|value|temp)$", "", key)
        if temp is not None and sensor_path not in seen_paths:
            seen_paths.add(sensor_path)
            temps.append(temp)
    if not temps:
        return None, None
...
def energy_counter_to_mw(value: Any, interval_s: float) -> float | None:
    ...
    return number / max(interval_s, 0.001)
...
def watts_value_to_mw(value: Any) -> float | None:
    number, unit = parse_value(value)
    if number is None or number < 0:
        return None
    if unit in {"mw", "mws"}:
...
def sample_duration_seconds(obj: dict[str, Any], requested_interval_s: float) -> float:
    elapsed_ns = dict_number(obj, "elapsed_ns")
    if elapsed_ns is None:
        elapsed_ns = find_best(list(flatten(obj)), ("elapsed ns",))
    if elapsed_ns is not None:
        elapsed_s = elapsed_ns / 1_000_000_000.0
        if 0.001 <= elapsed_s <= 120.0:
            return elapsed_s
    return max(0.001, requested_interval_s)
...
def intel_package_dict(processor: dict[str, Any]) -> dict[str, Any] | None:
    packages = processor.get("packages")
...
def has_intel_processor_fields(processor: dict[str, Any]) -> bool:
    if any(key in processor for key in ("package_watts", "cpu_watts", "igpu_watts", "dram_watts")):
        return True
    package = intel_package_dict(processor)
    if not isinstance(package, dict):
        return False
    return any(key in package for key in ("average_num_cores", "cores_active_ratio", "gpu_active_ratio", "gpu_freq_hz", "igpu_freq_hz"))


def intel_cpu_usage_from_package(package: dict[str, Any] | None) -> float | None:
    if not isinstance(package, dict):
        return None
    active = as_pct(package.get("cores_active_ratio"))
    if active is not None:
        return active
    average_cores = dict_number(package, "average_num_cores")
    cores = package.get("cores")
    if not isinstance(cores, list):
        cores = package.get("cpus")
    if average_cores is not None and isinstance(cores, list) and cores:
        return clamp(average_cores / len(cores) * 100.0, 0.0, 100.0)
    return None


def intel_core_metrics_from_package(package: dict[str, Any] | None) -> list[CoreMetric]:
    if not isinstance(package, dict):
        return []
    cpus = package.get("cores")
    key_name = "core"
    if not isinstance(cpus, list):
        cpus = package.get("cpus")
        key_name = "cpu"
    if not isinstance(cpus, list):
        return []
    ...
    for index, cpu in enumerate(cpus):
        if not isinstance(cpu, dict):
            continue
        raw_cpu_id = first_non_none(dict_number(cpu, key_name), dict_number(cpu, "cpu"), dict_number(cpu, "cpu_id"))
        cpu_id = int(raw_cpu_id) if raw_cpu_id is not None else index
        active = first_non_none(
            as_pct(cpu.get("active_ratio")),
            idle_to_active_pct(cpu.get("idle_ratio")),
            idle_to_active_pct(cpu.get("c_state_ratio")),
        )
        freq = as_mhz("freq_hz", cpu.get("freq_hz"))
        logical_cpus = cpu.get("cpus")
        if freq is None and isinstance(logical_cpus, list):
            freq = max_present(as_mhz("freq_hz", logical.get("freq_hz")) for logical in logical_cpus if isinstance(logical, dict))
        cores.append(CoreMetric(label=f"C{cpu_id}", usage_pct=active, freq_mhz=freq))
    return cores


def intel_core_usage_average(cores: list[CoreMetric]) -> float | None:
    return average(core.usage_pct for core in cores if core.label.startswith("C"))


def intel_gpu_usage_from_misc(obj: dict[str, Any]) -> float | None:
    gpus = obj.get("GPU")
    ...
    for gpu in gpus:
        if not isinstance(gpu, dict):
            continue
        counters = gpu.get("misc_counters")
        if not isinstance(counters, dict):
            continue
        for name, value in counters.items():
            if "gpu busy" in normalize_key(str(name)):
                busy = as_pct(value)
                if busy is not None:
                    return busy
    return None
...
def intel_gpu_freq_from_gpu_list(obj: dict[str, Any]) -> float | None:
    gpus = obj.get("GPU")
    if not isinstance(gpus, list):
        return None
    for gpu in gpus:
        if not isinstance(gpu, dict):
            continue
        freq = first_non_none(
            as_mhz("freq_hz", gpu.get("freq_hz")),
            as_mhz("freq_mhz", gpu.get("freq_mhz")),
        )
        if freq is not None:
            return freq
    return None
...
def apply_intel_powermetrics(sample: MetricSample, obj: dict[str, Any]) -> None:
    processor = obj.get("processor")
    if not isinstance(processor, dict):
        return
    if not has_intel_processor_fields(processor):
        return

    sample.telemetry_source = "intel"
    sample.soc_power_mw = first_non_none(watts_value_to_mw(processor.get("package_watts")), sample.soc_power_mw)
    sample.cpu_power_mw = first_non_none(watts_value_to_mw(processor.get("cpu_watts")), sample.cpu_power_mw)
    sample.gpu_power_mw = first_non_none(watts_value_to_mw(processor.get("igpu_watts")), sample.gpu_power_mw)
    sample.p_freq_mhz = first_non_none(as_mhz("freq_hz", processor.get("freq_hz")), sample.p_freq_mhz)

    package = intel_package_dict(processor)
    if isinstance(package, dict):
        if not sample.cores:
            sample.cores.extend(intel_core_metrics_from_package(package))
        sample.cpu_usage_pct = first_non_none(
            intel_core_usage_average(sample.cores),
            intel_cpu_usage_from_package(package),
            sample.cpu_usage_pct,
        )
        if sample.cpu_usage_pct is not None and sample.cpu_usage_source is None:
            sample.cpu_usage_source = "powermetrics"
        sample.gpu_usage_pct = first_non_none(intel_gpu_usage_from_misc(obj), as_pct(package.get("gpu_active_ratio")), sample.gpu_usage_pct)
        sample.gpu_freq_mhz = first_non_none(
            intel_gpu_freq_from_gpu_list(obj),
            as_mhz("gpu_freq_hz", package.get("gpu_freq_hz")),
            as_mhz("igpu_freq_hz", package.get("igpu_freq_hz")),
            sample.gpu_freq_mhz,
        )
    else:
        sample.gpu_usage_pct = first_non_none(intel_gpu_usage_from_misc(obj), sample.gpu_usage_pct)
        sample.gpu_freq_mhz = first_non_none(intel_gpu_freq_from_gpu_list(obj), sample.gpu_freq_mhz)
...
def apply_structured_powermetrics(sample: MetricSample, obj: dict[str, Any], interval_s: float) -> None:
    ...
    if isinstance(processor, dict):
        apply_intel_powermetrics(sample, obj)
                ...
                if isinstance(cpus, list):
                    for cpu in cpus:
                        if isinstance(cpu, dict):
                            ...
                            if kind in {"e", "p"}:
                                raw_cpu_id = dict_number(cpu, "cpu")
                                cpu_id = int(raw_cpu_id) if raw_cpu_id is not None else len(sample.cores)
                                sample.cores.append(
                                    CoreMetric(
...
def sample_from_plist(obj: dict[str, Any], interval_s: float = 1.0) -> MetricSample:
    raw_flat = list(flatten(obj))
    cleaned_obj, invalid_count = without_invalid_blocks(obj)
    if cleaned_obj is None or not isinstance(cleaned_obj, dict):
        return MetricSample(raw_keys=len(raw_flat), warning="powermetrics marked the sample invalid")
    flat = list(flatten(cleaned_obj))
    sample = MetricSample(raw_keys=len(raw_flat))
    actual_interval_s = sample_duration_seconds(cleaned_obj, interval_s)
    apply_structured_powermetrics(sample, cleaned_obj, actual_interval_s)
    sample.memory_bandwidth_gbps = bandwidth_counters_from_plist(cleaned_obj)
    ...
    for path, value in flat:
        ...
        low_text = text.lower()
        tokens = set(key.split())
        if "perf limit reasons" in key and isinstance(value, str) and low_text not in {"", "none", "n/a", "0"}:
            reason = re.sub(r"\s+", " ", text).strip()
        elif "throttle" in tokens or "throttled" in tokens:
            number, _ = parse_value(value)
            active = low_text in {"1", "true", "yes", "active"} or (number is not None and number > 0)
            reason = key if active else ""
        elif "limit" in tokens and not tokens.intersection(static_limit_words):
            number, _ = parse_value(value)
            active = low_text in {"1", "true", "yes", "active"} or (number is not None and number > 0)
            reason = key if active else ""
        else:
    ...
    if invalid_count:
        noun = "block" if invalid_count == 1 else "blocks"
        sample.warning = f"powermetrics ignored {invalid_count} invalid {noun}"
