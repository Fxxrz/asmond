...
Intel Macs are experimental. Current development builds can read a smaller set of Intel `powermetrics` counters on tested hardware, but Apple Silicon remains the primary supported target.
...
Intel Mac support is experimental and intentionally conservative. On the tested Intel Mac mini, Asmond can read package/CPU/iGPU power, CPU/GPU load, clocks, RAM, swap, disk/network I/O and process data, while temperature sensors, fan speed, battery and USB-C charging telemetry may be unavailable depending on hardware and macOS. Unsupported counters stay hidden or show `n/a` rather than estimated values.
